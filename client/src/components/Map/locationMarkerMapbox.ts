...
function buildLocationEl(): { root: HTMLDivElement; cone: HTMLDivElement } {
  ...
  pulse.style.cssText = `
    ...
    animation:yipyip-location-pulse 2s ease-out infinite;
  `
...
function ensurePulseStyle() {
  if (document.getElementById('yipyip-location-style')) return
  const s = document.createElement('style')
  s.id = 'yipyip-location-style'
  s.textContent = `
    @keyframes yipyip-location-pulse {
...
export function attachLocationMarker(map: mapboxgl.Map, MarkerCtor: MarkerConstructor): LocationMarkerHandle {
  ...
  const ensureAccuracyLayer = () => {
    if (map.getSource('yipyip-location-accuracy')) return
    try {
      map.addSource('yipyip-location-accuracy', {
        type: 'geojson',
      ...
      map.addLayer({
        id: 'yipyip-location-accuracy',
        type: 'fill',
        source: 'yipyip-location-accuracy',
        paint: {
          'fill-color': '#3b82f6',
          'fill-opacity': 0.14,
  ...
  const setAccuracy = (p: GeoPosition) => {
    const src = map.getSource('yipyip-location-accuracy') as mapboxgl.GeoJSONSource | undefined
    if (!src) return
  ...
  const handle: LocationMarkerHandle = {
    ...
    destroy: () => {
      ...
      try {
        if (map.getLayer('yipyip-location-accuracy')) map.removeLayer('yipyip-location-accuracy')
        if (map.getSource('yipyip-location-accuracy')) map.removeSource('yipyip-location-accuracy')
      } catch { /* noop */ }
    },
  }
