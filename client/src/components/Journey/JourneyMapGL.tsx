...
function ensureJourneyPopupStyle() {
  if (document.getElementById('yipyip-journey-popup-style')) return
  const s = document.createElement('style')
  s.id = 'yipyip-journey-popup-style'
  s.textContent = `
    .mapboxgl-popup.yipyip-journey-popup,
    .maplibregl-popup.yipyip-journey-popup { pointer-events: none; animation: yipyip-journey-popup-in 180ms ease-out; }
    .mapboxgl-popup.yipyip-journey-popup .mapboxgl-popup-content,
    .maplibregl-popup.yipyip-journey-popup .maplibregl-popup-content {
    ...
    .mapboxgl-popup.yipyip-journey-popup.yipyip-dark .mapboxgl-popup-content,
    .maplibregl-popup.yipyip-journey-popup.yipyip-dark .maplibregl-popup-content {
      background: rgba(24, 24, 27, 0.88);
      border-color: rgba(255, 255, 255, 0.08);
      color: #FAFAFA;
    }
    .mapboxgl-popup.yipyip-journey-popup .mapboxgl-popup-tip,
    .maplibregl-popup.yipyip-journey-popup .maplibregl-popup-tip {
      border-top-color: rgba(255, 255, 255, 0.94);
      border-bottom-color: rgba(255, 255, 255, 0.94);
    }
    .mapboxgl-popup.yipyip-journey-popup.yipyip-dark .mapboxgl-popup-tip,
    .maplibregl-popup.yipyip-journey-popup.yipyip-dark .maplibregl-popup-tip {
      border-top-color: rgba(24, 24, 27, 0.88);
      border-bottom-color: rgba(24, 24, 27, 0.88);
    }
    .mapboxgl-popup.yipyip-journey-popup .mapboxgl-popup-close-button,
    .maplibregl-popup.yipyip-journey-popup .maplibregl-popup-close-button { display: none; }
    .yipyip-journey-popup-title {
      font-size: 13.5px;
      font-weight: 600;
    ...
    .mapboxgl-popup.yipyip-journey-popup.yipyip-dark .yipyip-journey-popup-title,
    .maplibregl-popup.yipyip-journey-popup.yipyip-dark .yipyip-journey-popup-title { color: #FAFAFA; }
    .yipyip-journey-popup-sub {
      display: flex;
    ...
    .mapboxgl-popup.yipyip-journey-popup.yipyip-dark .yipyip-journey-popup-sub,
    .maplibregl-popup.yipyip-journey-popup.yipyip-dark .yipyip-journey-popup-sub { color: #A1A1AA; }
    .yipyip-journey-popup-place {
      min-width: 0;
      overflow: hidden;
      text-overflow: ellipsis;
    }
    .yipyip-journey-popup-sep {
      flex: 0 0 auto;
      opacity: 0.55;
      font-weight: 500;
    }
    .yipyip-journey-popup-date { flex: 0 0 auto; }
    @keyframes yipyip-journey-popup-in {
      from { opacity: 0; }
      to { opacity: 1; }
    }
  `
...
function markerHtml(dayColor: string, dayLabel: number, highlighted: boolean): HTMLDivElement {
  ...
  inner.className = 'yipyip-journey-marker-inner'
...
const JourneyMapGL = forwardRef<JourneyMapGLHandle, Props>(function JourneyMapGL(
  ...
  const showPopup = useCallback((id: string) => {
    ...
    if (place) subParts.push(`<span class="yipyip-journey-popup-place">${place}</span>`)
    if (date) subParts.push(`<span class="yipyip-journey-popup-date">${date}</span>`)
    const subline = subParts.length === 2
      ? `${subParts[0]}<span class="yipyip-journey-popup-sep">\u00B7</span>${subParts[1]}`
      : subParts.join('')

    const html = `
      <div class="yipyip-journey-popup-title">${primary}</div>
      ${subline ? `<div class="yipyip-journey-popup-sub">${subline}</div>` : ''}
    `
    ...
    if (popupRef.current) {
      ...
      if (el) el.classList.toggle('yipyip-dark', !!darkRef.current)
    } else {
      popupRef.current = new gl.Popup({
        ...
        anchor: 'bottom',
        offset,
        className: `yipyip-journey-popup${darkRef.current ? ' yipyip-dark' : ''}`,
        maxWidth: '280px',
      })
  ...
  const setMarkerStyle = useCallback((id: string, highlighted: boolean) => {
    ...
    const currentInner = el.querySelector('.yipyip-journey-marker-inner') as HTMLDivElement | null
    ...
    const nextInner = next.querySelector('.yipyip-journey-marker-inner') as HTMLDivElement
