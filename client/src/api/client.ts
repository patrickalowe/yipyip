...
/**
 * Validate a response payload against its @yipyip/shared Zod schema — but only in
 ...
 * the FE adopts per domain as each backend module lands on @yipyip/shared.
 */
...
export function parseInDev<S extends z.ZodTypeAny>(schema: S, data: unknown, label: string): z.infer<S> {
  if (API_DEV) {
    ...
    if (!result.success) {
      console.warn(`[api] ${label}: response did not match the @yipyip/shared schema`, result.error.issues)
    }
  }
...
apiClient.interceptors.response.use(
    ...
    async (error) => {
      ...
      // HTML body (a JS redirect page) instead of a 302. yipyip's own 401s are
...
export const tripsApi = {
  ...
  photos: (id: number | string) => apiClient.get(`/trips/${id}/photos`).then(r => r.data),
...
export const pluginsApi = {
  ...
  // supplied by untrusted plugin code (the yipyipBridge forwards it verbatim), so it
