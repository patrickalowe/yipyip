...
yipyip includes a built-in [Model Context Protocol](https://modelcontextprotocol.io/) (MCP) server that lets AI
assistants — such as Claude Desktop, Cursor, or any MCP-compatible client — read and modify your trip data through a
structured API.

> **Note:** MCP is an addon that must be enabled by your yipyip administrator before it becomes available.
...
```json
{
  "mcpServers": {
    "yipyip": {
      ...
      "args": [
        "mcp-remote",
        "https://your-yipyip-instance.com/mcp"
      ]
    }
  }
}
...
4. Your browser opens yipyip's consent screen, where you choose which scopes (permissions) to grant.
...
> **Requirement:** The `APP_URL` environment variable must be set to your yipyip instance's public URL for OAuth
...
(`yipyipcs_` prefix) and fixed scopes that you define up front.
  ...
  "mcpServers": {
    "yipyip": {
      ...
      "args": [
        ...
        "https://your-yipyip-instance.com/mcp",
        "--header",
        "Authorization: Bearer yipyip_your_token_here"
      ]
    }
  }
}
...
yipyip's MCP server supports three authentication methods. OAuth 2.1 is the recommended path for all external clients.

| Method | Token prefix | Access level | TTL | Notes |
|--------|-------------|-------------|-----|-------|
| **OAuth 2.1** | `yipyipoa_` | Scoped (per-consent) | 1 hour | Recommended. Automatically refreshed via 30-day rolling refresh tokens (`yipyiprf_` prefix). Replay-detected rotation — replayed tokens cascade-revoke the entire chain. |
| **Static API token** | `yipyip_` | Full access | No expiry | **Deprecated.** Triggers deprecation warnings in AI clients. Will be removed in a future release. |
| **Web session JWT** | — | Full access | Session-based | Used internally by the yipyip web UI. Not intended for external clients. |
...
When connecting via OAuth 2.1, you grant specific scopes during the consent step. yipyip registers only the MCP tools
...
| **Demo mode restrictions**              | If yipyip is running in demo mode, all write operations through MCP are blocked.                                                                   |
...
Resources provide read-only access to your yipyip data. MCP clients can read these to understand the current state before
making changes.
...
|-----------------------|-------------------------------------------------|---------------------------------------------------------------------------------------|
| Trips                 | `yipyip://trips`                                  | All trips you own or are a member of                                                  |
| Trip Detail           | `yipyip://trips/{tripId}`                         | Single trip with metadata and member count                                            |
| Days                  | `yipyip://trips/{tripId}/days`                    | Days of a trip with their assigned places                                             |
| Places                | `yipyip://trips/{tripId}/places`                  | All places/POIs saved in a trip. Supports `?assignment=all\|unassigned\|assigned`     |
| Budget                | `yipyip://trips/{tripId}/budget`                  | Budget and expense items                                                              |
| Budget Per-Person     | `yipyip://trips/{tripId}/budget/per-person`       | Per-person totals and split breakdown                                                 |
| Budget Settlement     | `yipyip://trips/{tripId}/budget/settlement`       | Suggested transactions to settle who owes whom                                        |
| Packing               | `yipyip://trips/{tripId}/packing`                 | Packing checklist                                                                     |
| Packing Bags          | `yipyip://trips/{tripId}/packing/bags`            | Packing bags with their assigned members                                              |
| Reservations          | `yipyip://trips/{tripId}/reservations`            | Flights, hotels, restaurants, etc.                                                    |
| Day Notes             | `yipyip://trips/{tripId}/days/{dayId}/notes`      | Notes for a specific day                                                              |
| Accommodations        | `yipyip://trips/{tripId}/accommodations`          | Hotels/rentals with check-in/out details                                              |
| Members               | `yipyip://trips/{tripId}/members`                 | Owner and collaborators                                                               |
| Collab Notes          | `yipyip://trips/{tripId}/collab-notes`            | Shared collaborative notes                                                            |
| To-Dos                | `yipyip://trips/{tripId}/todos`                   | To-do items ordered by position                                                       |
| Categories            | `yipyip://categories`                             | Available place categories (for use when creating places)                             |
| Bucket List           | `yipyip://bucket-list`                            | Your personal travel bucket list                                                      |
| Visited Countries     | `yipyip://visited-countries`                      | Countries marked as visited in Atlas                                                  |
| Notifications         | `yipyip://notifications/in-app`                   | Your in-app notifications (most recent 50, unread first)                              |
...
|-----------------------|-------------------------------------------------|----------|---------------------------------------------------------------------|
| Atlas Stats           | `yipyip://atlas/stats`                            | Atlas    | Visited country counts and continent breakdown                      |
| Atlas Regions         | `yipyip://atlas/regions`                          | Atlas    | Manually visited sub-country regions                                |
| Collab Polls          | `yipyip://trips/{tripId}/collab/polls`            | Collab   | All polls for a trip with vote counts per option                    |
| Collab Messages       | `yipyip://trips/{tripId}/collab/messages`         | Collab   | Most recent 100 chat messages for a trip                            |
| Vacay Plan            | `yipyip://vacay/plan`                             | Vacay    | Full snapshot of your active vacation plan (members, years, config) |
| Vacay Entries         | `yipyip://vacay/entries/{year}`                   | Vacay    | All vacation day entries for the active plan and a specific year    |
| Vacay Holidays        | `yipyip://vacay/holidays/{year}`                  | Vacay    | Public holidays for the plan's configured region and year           |
| Journeys              | `yipyip://journeys`                               | Journey  | All journeys owned or contributed to by the current user            |
| Journey Detail        | `yipyip://journeys/{journeyId}`                   | Journey  | Single journey with entries, contributors, and linked trips         |
| Journey Entries       | `yipyip://journeys/{journeyId}/entries`           | Journey  | All entries in a journey (date, text, mood, linked trip)            |
| Journey Contributors  | `yipyip://journeys/{journeyId}/contributors`      | Journey  | Contributors (owner and collaborators) of a journey                 |

---

## Tools (read-write)

yipyip exposes tools organized by feature area. Use `get_trip_summary` as a starting point — it returns everything about a
...
| `token_auth_notice`  | Static token deprecation notice and migration guide. Only available in sessions authenticated with a legacy `yipyip_` token. |
...
Before writing anything to yipyip, do some research: look up what's worth
...
Once you have a solid plan, write the whole thing to yipyip:
- Create the trip
...
PDF of the generated trip: [./docs/yipyip-Generated-by-MCP.pdf](./docs/yipyip-Generated-by-MCP.pdf)
