...
<a href="https://demo.likeyipyip.com"><img alt="Demo" src="https://img.shields.io/badge/Demo-try-111827?style=for-the-badge" /></a>
&nbsp;
<a href="https://hub.docker.com/r/mauriceboe/yipyip"><img alt="Docker" src="https://img.shields.io/badge/Docker-ready-2496ED?style=for-the-badge" /></a>
&nbsp;
...
<img src="https://github.com/mauriceboe/yipyip-media/releases/download/readme-assets/Yipyip1.gif" alt="yipyip — 60-second tour" width="100%" />
...
<picture>
  ...
  <img src="docs/tiles/grid-desktop.svg" alt="yipyip feature tiles" width="100%" />
</picture>
...
- **MCP** — expose yipyip to AI assistants via OAuth 2.1
...
ENCRYPTION_KEY=$(openssl rand -hex 32) docker run -d -p 3000:3000 \
  ...
  -v ./data:/app/data -v ./uploads:/app/uploads mauriceboe/yipyip
```

Open `http://localhost:3000`. On first boot yipyip seeds an admin account — if you set `ADMIN_EMAIL`/`ADMIN_PASSWORD` those are used, otherwise the credentials are printed to the container log (`docker logs yipyip`).
...
services:
  app:
    image: mauriceboe/yipyip:latest
    container_name: yipyip
    read_only: true
    security_opt:
    ...
    environment:
      ...
      # - OIDC_CLIENT_ID=yipyip
      # - OIDC_CLIENT_SECRET=supersecret
      # - OIDC_DISPLAY_NAME=SSO
      # - OIDC_ADMIN_CLAIM=groups
      # - OIDC_ADMIN_VALUE=app-yipyip-admins
    volumes:
      - ./data:/app/data
      - ./uploads:/app/uploads
...
yipyip works as a Progressive Web App — no App Store needed.

1. Open yipyip in the browser (HTTPS required)
2. **iOS**: Share ▸ *Add to Home Screen*
3. **Android**: Menu ▸ *Install app* (or *Add to Home Screen*)

yipyip then launches fullscreen with its own icon, just like a native app.
...
```bash
docker pull mauriceboe/yipyip
docker rm -f yipyip
docker run -d --name yipyip -p 3000:3000 -v ./data:/app/data -v ./uploads:/app/uploads --restart unless-stopped mauriceboe/yipyip
```

> Not sure which paths you used? `docker inspect yipyip --format '{{json .Mounts}}'` before removing the container.
...
```bash
docker exec -it yipyip node --import tsx scripts/migrate-encryption.ts
```
...
For production, put yipyip behind a TLS-terminating reverse proxy. yipyip uses WebSockets for real-time sync, so the proxy **must** support WebSocket upgrades on `/ws`.
...
server {
    listen 80;
    server_name yipyip.yourdomain.com;
...
| `COOKIE_SECURE` | Controls the `secure` flag on the `yipyip_session` cookie. Auto-derived: on when `NODE_ENV=production` or `FORCE_HTTPS=true`. Escape hatch: set `false` to allow session cookies over plain HTTP. Not recommended in production. | auto |
| `SESSION_DURATION` | How long a login session stays valid when **"Remember me" is unchecked** (the default): sets the `yipyip_session` JWT `exp` and issues a browser-session cookie (cleared when the browser closes). Accepts `ms`-style strings: `1h`, `12h`, `7d`, `30d`, `90d`. Invalid values warn at startup and fall back to the default. | `24h` |
| `SESSION_DURATION_REMEMBER` | Session length when **"Remember me" is ticked** at login: a longer-lived JWT plus a persistent `yipyip_session` cookie that survives browser restarts. Same format and startup-fallback behaviour as `SESSION_DURATION`. | `30d` |
...
| `APP_URL` | Public base URL of this instance (e.g. `https://yipyip.example.com`). Required when OIDC is enabled; used as base for email notification links. | — |
| **OIDC / SSO** | | |
...
| `OIDC_DISCOVERY_URL` | Override the auto-constructed OIDC discovery endpoint (e.g. Authentik: `.../application/o/yipyip/.well-known/openid-configuration`) | — |
| **Initial setup** | | |
| `ADMIN_EMAIL` | Email for the first admin on initial boot. Must be set together with `ADMIN_PASSWORD`. If either is omitted a random password is printed to the server log. No effect once a user exists. | `admin@yipyip.local` |
| `ADMIN_PASSWORD` | Password for the first admin on initial boot. Pairs with `ADMIN_EMAIL`. | random |
| **Other** | | |
| `DEMO_MODE` | Enable demo mode (hourly data resets) | `false` |
| `UNSPLASH_ACCESS_KEY` | Optional Unsplash Access Key for trip-cover and place-image search. Without one, yipyip uses Unsplash's unauthenticated endpoint, which some datacenter/VPS IPs are blocked from. Get a free key at [unsplash.com/developers](https://unsplash.com/developers). Overrides any per-admin key set in Admin > Settings (where it can also be configured instead). | — |
...
- **Logs** — `./data/logs/yipyip.log` (auto-rotated)
...
yipyip is [AGPL v3](LICENSE). Self-host freely for personal or internal company use. If you modify and offer yipyip as a network service to third parties, your modifications must be open-sourced under the same licence.
