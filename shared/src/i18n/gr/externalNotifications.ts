...
const gr: NotificationLocale = {
  email: {
    footer: 'Λάβατε αυτό το μήνυμα επειδή έχετε ενεργοποιήσει τις ειδοποιήσεις στο yipyip.',
    manage: 'Διαχείριση προτιμήσεων στις Ρυθμίσεις',
    madeWith: 'Δημιουργήθηκε με',
    openYipyip: 'Άνοιγμα yipyip',
  },
  events: {
    trip_invite: (p) => ({
    ...
    vacay_invite: (p) => ({
      ...
      body: `Ο/Η ${p.actor} σας προσκάλεσε να συγχωνεύσετε τα σχέδια διακοπών σας. Ανοίξτε το yipyip για να αποδεχτείτε ή να απορρίψετε.`,
    }),
    collection_invite: (p) => ({
      title: 'Πρόσκληση σε συλλογή',
      body: `Ο/Η ${p.actor} σας προσκάλεσε να μοιραστείτε μια συλλογή. Ανοίξτε το yipyip για να αποδεχτείτε ή να απορρίψετε.`,
    }),
    photos_shared: (p) => ({
    ...
    version_available: (p) => ({
      title: 'Νέα έκδοση yipyip διαθέσιμη',
      body: `Η έκδοση yipyip ${p.version} είναι τώρα διαθέσιμη. Επισκεφθείτε τον πίνακα διαχείρισης για να ενημερώσετε.`,
    }),
  ...
  passwordReset: {
    ...
    greeting: 'Γεια σας',
    body: 'Λάβαμε ένα αίτημα επαναφοράς του κωδικού πρόσβασης για τον λογαριασμό σας στο yipyip. Κάντε κλικ στο παρακάτω κουμπί για να ορίσετε νέο κωδικό πρόσβασης.',
