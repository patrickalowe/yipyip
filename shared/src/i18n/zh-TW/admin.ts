...
const admin: TranslationStrings = {
  ...
  'admin.plugins.disabledBody':
    '外掛執行環境已關閉（YIPYIP_PLUGINS_ENABLED）。在管理員於伺服器配置中啟用之前，任何外掛都無法執行。',
  ...
  'admin.plugins.reviewedMeaning':
    '「已審核」代表 yipyip 維護者曾針對每個版本掃描此外掛是否含有惡意程式碼——並非檢查品質或功能是否正常。這不保證外掛無害。',
  ...
  'admin.plugins.security.isolationBody':
    '外掛以自身受限的獨立程序執行，只能讀取自己的檔案。它無法讀取你的資料庫、登入密鑰或加密金鑰，無法啟動其他程式，也無法在任何位置寫入檔案。它的介面在一個封閉的瀏覽器框架中執行，無法讀取你的工作階段 Cookie，也無法觸及其周圍的 yipyip 頁面。',
  'admin.plugins.security.permsTitle': '這些權限的含義',
  'admin.plugins.security.permsBody':
    '安裝前顯示的權限是 yipyip 在外掛執行期間強制執行的硬性上限——它絕對無法做出清單以外的任何動作。但這些權限告訴你外掛「能」做什麼，而非它「實際」做什麼。一個獲准讀取你行程並連線某伺服器的外掛，就能將你的行程傳送到該伺服器，所以請閱讀權限與對外主機，而不只是看說明。',
  'admin.plugins.security.limitsTitle': '我們無法保證的事',
  'admin.plugins.security.limitsBody':
    '這種隔離是一道強而有力的軟體邊界，但並非絕對。外掛僅以你核准的權限行事，因此在這些權限範圍內，它的行為可能與其說明不符，也可能將它合法持有的資料傳送到它所宣告的主機。yipyip 不會讀取或評判外掛的程式碼實際做了什麼。',
  ...
  'admin.plugins.security.reviewedBody':
    '已審核的外掛，代表 yipyip 維護者曾針對每個版本手動掃描惡意程式碼——檢查是否含有有害程式碼，而非它是否運作良好。這並不保證外掛無害。',
  'admin.plugins.security.trustTitle': '結論',
  'admin.plugins.security.trustBody':
    '安裝外掛就像安裝任何第三方應用程式：只加入你信任的作者所提供的程式碼，如有疑慮，請先自行檢查。yipyip 對第三方外掛概不承擔任何責任。',
  ...
  'admin.addons.subtitle': '啟用或停用功能以自定義你的 yipyip 體驗。',
  ...
  'admin.weather.description':
    'yipyip 使用 Open-Meteo 作為天氣資料來源。Open-Meteo 是免費的開源天氣服務——無需 API 金鑰。',
  ...
  'admin.github.support': '幫助我繼續開發 yipyip',
  'admin.github.error': '載入版本失敗',
  'admin.github.by': '作者',
  'admin.update.available': '有可用更新',
  'admin.update.text': 'yipyip {version} 已釋出。你當前使用的是 {current}。',
  'admin.update.button': '在 GitHub 檢視',
  'admin.update.install': '安裝更新',
  'admin.update.confirmTitle': '確定安裝更新？',
  'admin.update.confirmText': 'yipyip 將從 {current} 更新到 {version}。伺服器將自動重啟。',
  ...
  'admin.update.dockerText': '你的 yipyip 例項執行在 Docker 中。要更新到 {version}，請在伺服器上執行以下命令：',
  'admin.update.nonDockerText':
    '此 yipyip 例項並非執行在 Docker 中。要更新到 {version}，請重新執行你當初使用的安裝或更新方式——例如在 Proxmox Community Scripts 上，請從 LXC 主控臺執行更新：',
  ...
  'admin.passkey.rpIdHint':
    'Passkey 綁定的純網域，例如 yipyip.example.org。留空則從 APP_URL 推導。日後變更將使現有 Passkey 失效。',
  'admin.passkey.origins': '允許的來源',
  'admin.passkey.originsHint': '以逗號分隔的完整來源，例如 https://yipyip.example.org。留空則使用 APP_URL。',
