...
const uk: NotificationLocale = {
  email: {
    footer: 'Ви отримали це, оскільки увімкнули сповіщення в yipyip.',
    manage: 'Керувати налаштуваннями у Налаштуваннях',
    madeWith: 'Made with',
    openYipyip: 'Відкрити yipyip',
  },
  events: {
    trip_invite: (p) => ({
    ...
    vacay_invite: (p) => ({
      ...
      body: `${p.actor} запрошує вас об'єднати плани відпустки. Відкрийте yipyip, щоб прийняти або відхилити.`,
    }),
    collection_invite: (p) => ({
      title: 'Запрошення до колекції',
      body: `${p.actor} запрошує вас поділитися колекцією. Відкрийте yipyip, щоб прийняти або відхилити.`,
    }),
    photos_shared: (p) => ({
    ...
    version_available: (p) => ({
      title: 'Доступна нова версія yipyip',
      body: `yipyip ${p.version} тепер доступний. Перейдіть до панелі адміністратора для оновлення.`,
    }),
  ...
  passwordReset: {
    ...
    greeting: 'Привіт',
    body: 'Ми отримали запит на скидання пароля вашого облікового запису yipyip. Натисніть кнопку нижче, щоб встановити новий пароль.',
