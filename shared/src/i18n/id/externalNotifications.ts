...
const id: NotificationLocale = {
  email: {
    footer: 'Anda menerima ini karena Anda telah mengaktifkan notifikasi di yipyip.',
    manage: 'Kelola preferensi di Pengaturan',
    madeWith: 'Dibuat dengan',
    openYipyip: 'Buka yipyip',
  },
  events: {
    trip_invite: (p) => ({
    ...
    vacay_invite: (p) => ({
      ...
      body: `${p.actor} mengundang Anda untuk menggabungkan rencana liburan. Buka yipyip untuk menerima atau menolak.`,
    }),
    collection_invite: (p) => ({
      title: 'Undangan koleksi',
      body: `${p.actor} mengundang Anda untuk berbagi koleksi. Buka yipyip untuk menerima atau menolak.`,
    }),
    photos_shared: (p) => ({
    ...
    version_available: (p) => ({
      title: 'Versi yipyip baru tersedia',
      body: `yipyip ${p.version} sekarang tersedia. Kunjungi panel admin untuk memperbarui.`,
    }),
  ...
  passwordReset: {
    ...
    greeting: 'Halo',
    body: 'Kami menerima permintaan untuk menyetel ulang kata sandi akun yipyip Anda. Klik tombol di bawah untuk menetapkan kata sandi baru.',
