 ...
 * client) keeps the standard yipyip `{ error }` 404 envelope. The `@Catch(NotFoundException)`
 * is more specific than the global YipyipExceptionFilter, so Nest routes 404s here
 * while every other error still flows through YipyipExceptionFilter.
 */
...
export class SpaFallbackFilter implements ExceptionFilter {
    ...
    // Non-production, or a non-GET miss: keep the standard yipyip 404 envelope
    // (identical to what YipyipExceptionFilter produces for a NotFoundException).
