...
import { yipyipOAuthProvider, yipyipClientsStore } from '../../mcp/oauthProvider';
...
export function applyPlatformTransport(app: express.Application): void {
  ...
  // the yipyip home page instead of the consent form.
  ...
  app.use('/oauth/authorize', mcpAddonGate, authorizationHandler({ provider: yipyipOAuthProvider }));

  // SDK DCR handler: accepts registrations without scope (fixes issue #959 bug 2)
  app.use('/oauth/register', mcpAddonGate, clientRegistrationHandler({ clientsStore: yipyipClientsStore }));
