...
// Version check: daily at 9 AM — notify admins if a new yipyip release is available
...
// The TTL must exceed any realistic offline window: the yipyip client replays
...
// yipyip photo cache cleanup: every 2 hours — evict disk files and DB rows past their 1h TTL
let yipyipPhotoCacheTask: ScheduledTask | null = null;

function startYipyipPhotoCacheCleanup(): void {
  if (yipyipPhotoCacheTask) { yipyipPhotoCacheTask.stop(); yipyipPhotoCacheTask = null; }

  // Run once immediately on startup to evict any entries left over from a previous run
  try {
    const { sweepExpired } = require('./services/memories/yipyipPhotoCache');
    sweepExpired();
  } catch { /* cache dir may not exist yet — harmless */ }

  yipyipPhotoCacheTask = cron.schedule('0 */2 * * *', () => {
    try {
      const { sweepExpired } = require('./services/memories/yipyipPhotoCache');
      sweepExpired();
    } catch (err: unknown) {
      logError(`yipyip photo cache cleanup: ${err instanceof Error ? err.message : err}`);
    }
  });
}
...
export { start, stop, startDemoReset, startTripReminders, startTodoReminders, startVersionCheck, startIdempotencyCleanup, purgeExpiredIdempotencyKeys, startYipyipPhotoCacheCleanup, startPlacePhotoCacheCleanup, startAirTrailSync, loadSettings, saveSettings, VALID_INTERVALS };
