...
export function extractToken(req: Request): string | null {
  ...
  const cookieToken = (req as any).cookies?.yipyip_session;
...
const demoUploadBlock = (req: Request, res: Response, next: NextFunction): void => {
  ...
  if (process.env.DEMO_MODE?.toLowerCase() === 'true' && isDemoEmail(authReq.user?.email)) {
    res.status(403).json({ error: 'Uploads are disabled in demo mode. Self-host yipyip for full functionality.' });
    return;
  }
  next();
};
