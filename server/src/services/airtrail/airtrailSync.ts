 ...
 * updated_at) and, when a flight is gone from AirTrail, keep the yipyip row but
 ...
 * GET object) and overwrite ONLY the fields yipyip manages. Everything else —
...
export function buildSavePayload(reservation: any, existing: AirtrailFlightRaw): AirtrailSavePayload | null {
  ...
  // Push the seat the user set in yipyip onto their own AirTrail seat (the one with
  ...
  // overwrite only what yipyip manages. `from`/`to`/`airline`/`aircraft` come back
  ...
  return {
    ...
    // Import reads the SCHEDULED time, so a yipyip edit must write back there too —
    ...
    // These are AirTrail-owned details yipyip doesn't surface in its edit UI — a yipyip
    // edit can leave them out of `metadata`. Preserve AirTrail's current value when
    // yipyip has none rather than nulling it out (#1240). Use airline_code (not the
