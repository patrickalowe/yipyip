...
function generateAccessToken(): string {
  return 'yipyipoa_' + randomBytes(32).toString('hex');
}

function generateRefreshToken(): string {
  return 'yipyiprf_' + randomBytes(32).toString('hex');
}
...
export function createOAuthClient(
  ...
  const rawSecret   = isPublic ? null : 'yipyipcs_' + randomBytes(24).toString('hex');
...
export function rotateOAuthClientSecret(
  ...
  const rawSecret  = 'yipyipcs_' + randomBytes(24).toString('hex');
...
export function validateAuthorizeRequest(
  ...
  // RFC 8707 resource indicator: if provided, must identify the yipyip
  ...
  if (resource !== mcpResource) {
    return { valid: false, error: 'invalid_target', error_description: 'Requested resource must be the yipyip MCP endpoint' };
  }
