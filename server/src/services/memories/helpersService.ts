...
export function canAccessUserPhoto(requestingUserId: number, ownerUserId: number, tripId: string, assetId: string, provider: string): boolean {
    ...
    if (tripId === '0') {
        const journeyPhoto = db.prepare(`
            ...
            JOIN yipyip_photos tkp ON tkp.id = gp.photo_id
            WHERE tkp.asset_id = ?
              AND tkp.provider = ?
              AND tkp.owner_id = ?
            LIMIT 1
    ...
    // Regular trip photos — join through yipyip_photos
...
// ── Unified photo access check (yipyip_photos based) ──────────────────────

export function canAccessYipyipPhoto(requestingUserId: number, yipyipPhotoId: number): boolean {
    const photo = db.prepare('SELECT * FROM yipyip_photos WHERE id = ?').get(yipyipPhotoId) as { id: number; provider: string; owner_id: number | null } | undefined;
    ...
    const tripAccess = db.prepare(`
        ...
        LIMIT 1
    `).get(yipyipPhotoId, requestingUserId, requestingUserId);
