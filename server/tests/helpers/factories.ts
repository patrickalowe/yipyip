...
export function createMcpToken(
  ...
  const rawToken = overrides.rawToken ?? `yipyip_test_${Date.now()}_${Math.random().toString(36).slice(2)}`;
...
export function addTripPhoto(
  ...
  // Insert into yipyip_photos first (central registry)
  db.prepare(
    'INSERT OR IGNORE INTO yipyip_photos (provider, asset_id, owner_id) VALUES (?, ?, ?)'
  ).run(provider, assetId, userId);
  const yipyipPhoto = db.prepare(
  ...
  ).run(tripId, userId, yipyipPhoto.id, opts.shared ? 1 : 0, opts.albumLinkId ?? null);
  return db.prepare(`
