...
describe('resolveOidcRole', () => {
  ...
  it('OIDC-SVC-011: returns admin when groups array contains OIDC_ADMIN_VALUE', () => {
    process.env.OIDC_ADMIN_VALUE = 'yipyip-admins';
    expect(resolveOidcRole({ sub: 'x', groups: ['yipyip-users', 'yipyip-admins'] }, false)).toBe('admin');
  });

  it('OIDC-SVC-012: returns user when groups array does not contain OIDC_ADMIN_VALUE', () => {
    process.env.OIDC_ADMIN_VALUE = 'yipyip-admins';
    expect(resolveOidcRole({ sub: 'x', groups: ['yipyip-users'] }, false)).toBe('user');
  });
...
describe('verifyIdToken', () => {
  ...
  const ISSUER = 'https://auth.example.com/application/o/yipyip';
  const CLIENT_ID = 'yipyip-client';
