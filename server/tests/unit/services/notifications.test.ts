...
describe('buildWebhookBody', () => {
  ...
  it('Discord embed footer defaults to yipyip when no trip name', () => {
    ...
    expect(body.embeds[0].footer.text).toBe('yipyip');
  });
  ...
  it('generic URL produces plain JSON with original fields plus timestamp and source', () => {
    ...
    expect(body).toHaveProperty('source', 'yipyip');
  });
});
...
describe('buildEmailHtml', () => {
  ...
  it('uses English i18n strings for lang=en', () => {
    ...
    expect(html).toContain('notifications enabled in yipyip');
  });

  it('uses German i18n strings for lang=de', () => {
    const html = buildEmailHtml('Subject', 'Body', 'de');
    expect(html).toContain('yipyip aktiviert');
  });
  ...
  it('falls back to English i18n for unknown language', () => {
    ...
    expect(unknown).toContain('notifications enabled in yipyip');
  });
});
...
describe('sendNtfy', () => {
  const ntfyUrl = 'https://ntfy.sh/yipyip-test';
  ...
  it('NTFY-004 — includes Click header when link is provided', async () => {
    ...
    await sendNtfy(ntfyUrl, null, { ...payload, link: 'https://yipyip.example.com/trips/5' });

    const [, calledOpts] = mockFetch.mock.calls[0];
    expect(calledOpts.headers['Click']).toBe('https://yipyip.example.com/trips/5');
  });
