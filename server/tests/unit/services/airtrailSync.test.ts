...
/**
 * An AirTrail flight as GET returns it, including the fields yipyip doesn't model.
...
function existingFlight(
  ...
  return {
    ...
    // AirTrail-owned detail yipyip never surfaces — must survive a writeback (#1240).
...
/** A linked yipyip reservation (the shape getReservationWithJoins returns). */
...
describe('airtrailSync.buildSavePayload', () => {
  it('round-trips the AirTrail-owned fields yipyip does not model (issue #1240)', () => {
  ...
  it('writes the yipyip time to the SCHEDULED fields so it round-trips on the next pull', () => {
    // Import reads the scheduled time, so a yipyip edit must be pushed back there
  ...
  it('blanks the scheduled time when the yipyip reservation has only a date', () => {
  ...
  it('still applies the yipyip-owned edits on top of the preserved fields', () => {
    const payload = buildSavePayload(
      reservation({
        reservation_time: '2021-09-01T20:30',
        notes: 'changed in yipyip',
  ...
  it('preserves AirTrail aircraft/airline/reason when yipyip metadata omits them (#1240)', () => {
    // A yipyip edit can drop these AirTrail-owned fields from metadata; the writeback
