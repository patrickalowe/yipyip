 ...
 * Also covers the resource yipyip://notifications/in-app.
 */
...
// ---------------------------------------------------------------------------
// Resource: yipyip://notifications/in-app
// ---------------------------------------------------------------------------

describe('Resource: yipyip://notifications/in-app', () => {
  it('returns notifications list', async () => {
    const { user } = createUser(testDb);
    createNotification(testDb, user.id, { title_key: 'notif.test' });
    await withResourceHarness(user.id, async (h) => {
      const result = await h.client.readResource({ uri: 'yipyip://notifications/in-app' });
