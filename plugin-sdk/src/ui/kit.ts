/**
 * The yipyip plugin design kit (#plugins).
 *
 * A plugin's UI runs in a sandboxed, opaque-origin iframe — it can't load yipyip's
 * stylesheet, only postMessage. So instead of forcing every author to re-derive the
 * look, we ship it: a token-driven stylesheet (`YIPYIP_UI_CSS`) plus a tiny bootstrap
 * (`YIPYIP_THEME_JS`) that wires the frame to the host. Both are plain strings, meant
 ...
 * `<!-- yipyip:ui -->` marker; `dev`/`pack` expand it, and `create` seeds it.
 *
 ...
 * `.yipyip-dash` layer (the --glass, --r and --sh families) is scoped to the dashboard
...
export const YIPYIP_UI_MARKER = '<!-- yipyip:ui -->';

/** Token-driven stylesheet. Inline as `<style>${YIPYIP_UI_CSS}</style>`. */
export const YIPYIP_UI_CSS = `/* yipyip plugin design kit — token-driven, matches the host in light + dark. */
:root {
  color-scheme: light;
  ...
  /* Baked glass layer (mirrors the host's .yipyip-dash; not sent over the bridge). */
  ...
  /* The house easings: a punchy card curve and yipyip's ease-out-quint. */
  --yipyip-ease: cubic-bezier(.2,.7,.2,1);
  --yipyip-ease-quint: cubic-bezier(.23,1,.32,1);
}
[data-theme="dark"] {
  color-scheme: dark;
...
/* Base: a light reset + native type. The bootstrap adds \`yipyip-ui\` to <body>. */
...
.yipyip-ui :focus-visible { outline: 2px solid var(--accent); outline-offset: 2px; border-radius: 4px; }
...
.yipyip-interactive {
  transition: transform .3s var(--yipyip-ease), box-shadow .3s, border-color .3s;
  cursor: pointer;
}
.yipyip-glass.yipyip-interactive:hover {
  transform: translateY(-2px);
  box-shadow: var(--glass-shadow-hover), var(--glass-highlight);
}
.yipyip-card.yipyip-interactive:hover { transform: translateY(-2px); box-shadow: var(--shadow-md); }
.yipyip-interactive:active { transform: translateY(0); }
...
.yipyip-btn {
  ...
  transition: transform .08s var(--yipyip-ease-quint), background .15s, box-shadow .15s, border-color .15s, color .15s;
}
.yipyip-btn:active { transform: scale(.97); }
.yipyip-btn:disabled { opacity: .5; cursor: not-allowed; }
.yipyip-btn--primary { background: var(--accent); color: var(--accent-text); box-shadow: var(--shadow-sm); }
.yipyip-btn--primary:hover:not(:disabled) { background: var(--accent-hover); }
.yipyip-btn--secondary { background: var(--bg-card); color: var(--text-primary); border-color: var(--border-primary); box-shadow: var(--shadow-sm); }
.yipyip-btn--secondary:hover:not(:disabled) { background: var(--bg-hover); }
.yipyip-btn--ghost { background: transparent; color: var(--text-secondary); }
.yipyip-btn--ghost:hover:not(:disabled) { background: var(--bg-hover); color: var(--text-primary); }
.yipyip-btn--danger { background: var(--danger); color: #fff; }
.yipyip-btn--danger:hover:not(:disabled) { filter: brightness(1.05); }

/* Form controls ------------------------------------------------------------ */
.yipyip-input, .yipyip-textarea, .yipyip-select {
...
.yipyip-textarea { resize: vertical; min-height: 72px; }
.yipyip-input::placeholder, .yipyip-textarea::placeholder { color: var(--text-faint); }
.yipyip-input:focus, .yipyip-textarea:focus, .yipyip-select:focus {
  border-color: var(--accent);
  box-shadow: 0 0 0 3px color-mix(in oklch, var(--accent) 22%, transparent);
}
.yipyip-label { display: block; font-size: 12px; font-weight: 600; color: var(--text-secondary); margin-bottom: 6px; }
...
.yipyip-chip--accent  { color: var(--accent);  background: color-mix(in oklch, var(--accent) 12%, transparent); }
.yipyip-chip--success { color: var(--success); background: var(--success-soft); }
.yipyip-chip--danger  { color: var(--danger);  background: var(--danger-soft); }
.yipyip-chip--warning { color: var(--warning); background: var(--warning-soft); }
.yipyip-chip--info    { color: var(--info);    background: var(--info-soft); }
...
.yipyip-row:hover { background: var(--bg-hover); }
.yipyip-title { font-size: 13px; font-weight: 500; text-transform: uppercase; letter-spacing: .14em; color: var(--text-muted); }
.yipyip-muted { color: var(--text-muted); }
.yipyip-faint { color: var(--text-faint); }

/* Layout helpers ----------------------------------------------------------- */
.yipyip-stack { display: flex; flex-direction: column; gap: 12px; }
.yipyip-cluster { display: flex; align-items: center; flex-wrap: wrap; gap: 8px; }

/* Accessibility: mirror the host's own graceful-degrade rules. -------------- */
[data-no-transparency] .yipyip-glass {
  background: var(--bg-card); border-color: var(--border-primary);
  box-shadow: var(--shadow-card);
  -webkit-backdrop-filter: none; backdrop-filter: none;
}
[data-reduce-motion] .yipyip-interactive,
[data-reduce-motion] .yipyip-btn,
[data-reduce-motion] .yipyip-row { transition: none; }
[data-reduce-motion] .yipyip-interactive:hover,
[data-reduce-motion] .yipyip-btn:active { transform: none; }
@media (prefers-reduced-motion: reduce) {
  .yipyip-interactive, .yipyip-btn, .yipyip-row, .yipyip-input, .yipyip-textarea, .yipyip-select { transition: none; }
  .yipyip-interactive:hover, .yipyip-btn:active { transform: none; }
}`;

/**
 * The bridge bootstrap. Inline as `<script>${YIPYIP_THEME_JS}</script>` (typically via
 * the `<!-- yipyip:ui -->` marker). It: announces readiness; applies the host's theme
 * tokens, theme name and appearance flags to the document; auto-reports its height so
 * a widget/page self-sizes; and installs a small `window.yipyip` helper over the same
 * bridge messages the host already understands — it adds no new capability.
 */
export const YIPYIP_THEME_JS = `(function () {
  'use strict';
  ...
  function applyContext(m) {
    ...
    if (document.body) { document.body.classList.add('yipyip-ui'); }
  }

  function reportHeight() {
    var h = Math.ceil(document.documentElement.scrollHeight);
    if (h > 0 && h !== lastH) { lastH = h; send({ type: 'yipyip:resize', height: h }); }
  }
  ...
  // hand-written CSS — every element carries the same yipyip-* classes the kit ships.
  ...
  var ui = {
    ...
    button: function (label, opts) {
      opts = opts || {};
      return mkEl('button', { class: 'yipyip-btn' + (opts.variant ? ' yipyip-btn--' + opts.variant : ''), type: 'button', text: label, on: opts.onClick ? { click: opts.onClick } : null }, null);
    },
    card: function (children) { return mkEl('div', { class: 'yipyip-card' }, children); },
    chip: function (text, variant) { return mkEl('span', { class: 'yipyip-chip' + (variant ? ' yipyip-chip--' + variant : ''), text: text }, null); },
    input: function (opts) { opts = opts || {}; return mkEl('input', { class: 'yipyip-input', type: opts.type || 'text', placeholder: opts.placeholder || '', value: opts.value || '' }, null); },
  ...
  var api = {
    context: null,
    ui: ui,
    ready: function () { send({ type: 'yipyip:ready' }); },
    requestContext: function () { send({ type: 'yipyip:context:request' }); },
    ...
    notify: function (level, message) { send({ type: 'yipyip:notify', level: level, message: message }); },
    navigate: function (to) { send({ type: 'yipyip:navigate', to: to }); },
    resize: function (px) { var h = px | 0; if (h > 0) { lastH = h; send({ type: 'yipyip:resize', height: h }); } },
    invoke: function (sub, opts) {
      opts = opts || {};
      var id = 'r' + (++seq);
      return new Promise(function (resolve, reject) {
        pending[id] = { resolve: resolve, reject: reject };
        send({ type: 'yipyip:invoke', requestId: id, sub: sub, method: opts.method, body: opts.body });
      });
    }
  };
  window.yipyip = api;
...
/**
 * Replace the `<!-- yipyip:ui -->` marker in a plugin's HTML with the inlined kit
 ...
 */
export function injectYipyipUi(html: string): string {
  if (!html.includes(YIPYIP_UI_MARKER)) return html;
  const block = `<style data-yipyip-ui>\n${YIPYIP_UI_CSS}\n</style>\n<script data-yipyip-ui>\n${YIPYIP_THEME_JS}\n</script>`;
  return html.split(YIPYIP_UI_MARKER).join(block);
}
