 ...
 *
 * Turns a built plugin directory into the exact plugin.zip the yipyip installer
...
const ROOT_FILES = ['yipyip-plugin.json', 'README.md', 'LICENSE', 'LICENSE.md', 'package.json'];
...
function walk(base: string, rel: string, out: ZipInput[]): void {
  for (const entry of fs.readdirSync(path.join(base, rel), { withFileTypes: true })) {
    ...
    if (entry.isDirectory()) {
      ...
      // Expand the `<!-- yipyip:ui -->` marker into the inlined design kit as the file
      ...
      if (entry.name.endsWith('.html')) {
        out.push({ name: childRel, data: Buffer.from(injectYipyipUi(fs.readFileSync(path.join(base, childRel), 'utf8')), 'utf8') });
      } else {
...
export function packPluginDir(dir: string, outPath: string): PackResult {
  ...
  if (!filtered.some((f) => f.name === 'yipyip-plugin.json')) throw new Error('yipyip-plugin.json must be at the plugin root');
