...
| Mirror journey photos to Immich on upload | No | Checkbox; when enabled, photos you upload in yipyip are also pushed to your Immich library |

Enter the full URL of your Immich instance and an Immich API key. The API key is stored encrypted on the yipyip server and is never returned to the browser after it is saved.

#### Required API key permissions

When generating the API key in Immich (**Account Settings → API Keys**), grant only the scopes yipyip actually uses:

| Permission | Why yipyip needs it |
|------------|-------------------|
...
| `asset.upload` | *Only if you enable "Mirror journey photos to Immich on upload"* — push yipyip uploads back to your library |

yipyip never modifies or deletes anything in Immich, so no `update`, `delete`, or admin scopes are needed.
...
Synology Photos doesn't use API keys — yipyip signs in with a regular DSM user account. To minimize blast radius, create a **dedicated low-privilege DSM user** for yipyip rather than reusing your admin account:
...
- 2FA is supported — enter the OTP at first connection; yipyip stores the resulting device token so you won't be re-prompted on subsequent saves.
- Read-only access is enough — yipyip only lists albums, lists items, runs searches, and fetches thumbnails. It never writes, uploads, or deletes.
...
You can configure both Immich and Synology simultaneously. yipyip queries photos from all enabled providers when loading trip photos.
