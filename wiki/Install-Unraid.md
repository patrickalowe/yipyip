...
Install yipyip on Unraid via Community Applications or a direct template import.
...
| Data | `/app/data` | `/mnt/user/appdata/yipyip/data` |
| Uploads | `/app/uploads` | `/mnt/user/appdata/yipyip/uploads` |
...
| `ALLOWED_ORIGINS` | *(empty)* | Comma-separated origins for CORS and email notification links, e.g. `https://yipyip.example.com` |
...
| `ADMIN_PASSWORD` | *(empty)* | Password for the first admin account (first-boot only). Must be set together with `ADMIN_EMAIL`. If either is omitted, yipyip creates the account with email `admin@yipyip.local` and a random password printed to the container log. |
...
Copy the output into the `ENCRYPTION_KEY` field before starting the container for the first time. If you skip this, yipyip auto-generates a key and saves it to `data/.encryption_key` — your data is still protected, but you should record that file in your backups.
...
On first boot, yipyip automatically creates an admin account. The credentials are printed to the container log — check **Docker → yipyip → Log** in the Unraid UI. If you set both `ADMIN_EMAIL` and `ADMIN_PASSWORD`, those values are used; otherwise the email is `admin@yipyip.local` and a random password is generated.
