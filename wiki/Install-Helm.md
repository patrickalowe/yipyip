...
Deploy yipyip on Kubernetes using the official Helm chart.
...
This deploys yipyip with default values: a `ClusterIP` service on port 3000, 1 Gi PVCs for data and uploads, and no ingress.
...
```bash
helm install yipyip yipyip/yipyip --set generateEncryptionKey=true
```
...
```bash
kubectl create secret generic yipyip-secrets \
  --from-literal=ENCRYPTION_KEY=$(openssl rand -hex 32)

helm install yipyip yipyip/yipyip \
  --set existingSecret=yipyip-secrets
```
...
ingress:
  ...
  hosts:
    - host: yipyip.example.com
      paths:
        - /
  tls:
    - secretName: yipyip-tls
      hosts:
        - yipyip.example.com
```

> **Important:** yipyip uses WebSockets on `/ws`. Your ingress controller must support WebSocket upgrades. Set `proxy-read-timeout` to at least `86400` and `proxy-body-size` to at least `500m` for backup restores.
...
```bash
helm repo update
helm upgrade yipyip yipyip/yipyip
```
